...
function ViewCourse({params}) {
    ...
    const getCourse=async()=>{
        await getCourseById(params?.courseId,user.primaryEmailAddress.emailAddress).then(resp=>{
  ...
  return course?.name&&(
    <div className='flex'>
      ...
      <div className='hidden fixed bg-white md:block md:w-80 border shadow-sm h-screen z-50'>
        {course? <ChapterNav course={course} userCourse={userCourse} setActiveChapter={(chapter)=>setActiveChapter(chapter)}/>:null}
      </div>
      <div className='ml-80'>
